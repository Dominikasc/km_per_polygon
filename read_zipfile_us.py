...
if uploaded_files != []:
    ...
    with col2:
        ...
        numeric_cols = ['Miles per area', 'Miles per year', 'Hours per year']
        ...
        table_poly_view = table_poly_view.astype({
            'Line': str,
            'Area': str,
            'Miles per area': float,
            'Miles per year': float,
        ...
        totals_row = pd.DataFrame([{
            ...
            'Miles per area': table_poly_view['Miles per area'].astype(float).sum(),
            'Miles per year': table_poly_view['Miles per year'].astype(float).sum(),
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Miles per area": st.column_config.NumberColumn(
                    "Miles/Area",
                    format="%.2f",
                    width="dynamic"
                ),
                "Miles per year": st.column_config.NumberColumn(
                    "Miles/Year",
                    format="%.2f",
                    width="dynamic"
                ),
